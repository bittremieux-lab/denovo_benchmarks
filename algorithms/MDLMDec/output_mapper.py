...
import sys
sys.path.append("/cmnfs/home/j.lapin/projects/denovo_benchmarks/algorithms")
import argparse
import os
import re
import numpy as np
...
#output_data['length'] = output_data['prediction'].map(lambda x: len(x))

# Simulate aa_scores
output_data['aa_scores'] = output_data['aa_scores'].map(lambda x: ",".join([str(m) for m in x]))
...
output_data['scans'] = output_data['spectrum_id'].map(lambda x: x.split(':')[0])
